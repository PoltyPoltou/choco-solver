...
import org.chocosolver.solver.constraints.nary.knapsack.innernode.factory.InnerNodeFactory;

public class BinarySearchFingerTree extends FingerTree<InnerNode, KPItem> {

    public BinarySearchFingerTree(List<KPItem> sortedItems, InnerNodeFactory factory) {
        super(sortedItems);
        setupTree(factory);
    }
    ...
    private void setupTree(InnerNodeFactory factory) {
        ...
        for (int i = 0; i < innerNodeSize; ++i) {
            getInnerNodeTreeList().set(i, factory.createInnerNode());
        }
